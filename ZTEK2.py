...
def courbe(pourcent_chercher2,tiker_live,time1,time_name1,pourcent_chercher,pourcent_perdu,note,debugage,dejatoucher2,mirande3,mirande2,J,I,moyenne_tete,moins50p,local_min,local_max,A,B,C,D,E,F,G,df,place_liveprice):
    ...
    plt.axhline(y=j[1] -  ((moyenne_tete) * 80 / 100), linestyle='--', alpha=0.3, color='orange',
                label='80% objectif')
    plt.axhline(y=j[1] -  (moyenne_tete) / 4, linestyle='--', alpha=0.3, color='black',
                label='25% objectif')
    plt.axhline(y=moins50p, linestyle='--', alpha=0.3, color='purple', label='-60% objectif')
    plt.grid(True, which='major', color='#666666', linestyle='-', alpha=0.1)
    taille_diviser = (local_min[-1] - local_min[-2]) / (local_max[-2] - local_min[-2])
    ...
    plt.legend()
    plt.text(local_min[-3], A, "A", ha='left', style='normal', size=10.5, color='red',
             wrap=True)
    plt.text(J[0], j[1] -  ((moyenne_tete) * 80 / 100), f"{round((j[1] -  (moyenne_tete) / 2), 5)}",
    ...
    plt.text(J[0], moins50p, f"{round(moins50p, 5)}", ha='left', style='normal', size=10.5,
             color='red', wrap=True)
    plt.text(local_max[-3], B, "B", ha='left', style='normal', size=10.5, color='red',
             wrap=True)
    plt.text(local_min[-2], C, "C", ha='left', style='normal', size=10.5, color='red',
             wrap=True)
    plt.text(local_max[-2], D, f"D {round(D, 5)}", ha='left', style='normal', size=10.5,
             color='red', wrap=True)
    plt.text(local_min[-1], E, "E", ha='left', style='normal', size=10.5, color='red',
             wrap=True)
    plt.text(local_max[-1], F, f"F  {round(F, 5)}", ha='left', style='normal', size=10.5,
             color='red', wrap=True)
    ...
    plt.scatter(local_min[-3], A, color='blue')
    plt.scatter(local_max[-3], B, color='blue')
    plt.scatter(local_min[-2], C, color='blue')
    plt.scatter(local_max[-2], D, color='blue')
    plt.scatter(local_min[-1], E, color='blue')
    plt.scatter(local_max[-1], F, color='blue')
...
def Finder_IETE(time1, time_name1, start1):
    ...
    if passed1 == True:
        ...
        try:
            ...
            B = float(df['c'].iloc[local_max[-3]])
            C = float(df['c'].iloc[local_min[-2]])
            D = float(df['c'].iloc[local_max[-2]])
            E = float(df['c'].iloc[local_min[-1]])
            F = float(df['c'].iloc[local_max[-1]])
            G = float(livePrice)
            ...
            #if (B - C) < (D - C) and (B - C) < (D - E) and (F - E) < (D - E) and (F - E) < (
            #        D - C) and D > B and D > F and C < B and E < F and local_min[-3] < local_max[-3] < local_min[-2] < local_max[-2] < local_min[-1] < local_max[-1] and ((D - ((C + E) / 2)) * 100) / D >= 2.8 and moyenne_epaule1 <= moyenne_tete / 2 and moyenne_epaule2 <= moyenne_tete / 2 and moyenne_epaule1 >= moyenne_tete / 4 and moyenne_epaule2 >= moyenne_tete / 4 :
            #    fig = plt.figure(figsize=(10, 7))
            #    plt.plot([], [], ' ')
            #    plt.title(f'IETE : debug {tiker_live}', fontweight="bold", color='black')
...
#
        ...
        if passed2 == True:
            ...
            vert.append(local_min[-3])
            vert.append(local_min[-2])
            vert.append(local_min[-1])
            vert.append(place_liveprice)

            i = 0
            passed3 = False
            try:
                for i in range(local_min[-4] - 1, len(df)):
                    bleu.append(i)
                    passed3 = True
            except:
            ...
            if passed3 == True:
                ...
                if E < C:
                    ...
                    mirande2['c'].values[3] = mirande2['c'].values[2] - ((suite * (place_liveprice - local_max[-1])))
                if E == C:
                    mirande2['c'].values[0] = df['c'].values[local_min[-2]]
                    mirande2['c'].values[3] = df['c'].values[local_min[-1]]
                # ----- determiner la direction pente LDC et allongement apres E et C -----#
                ...
                AI = [local_min[-3], mirande2['c'].iloc[0]]
                BI = [local_min[-2], mirande2['c'].iloc[1]]
                ...
                AJ = [local_min[-1], mirande2['c'].iloc[2]]
                ...
                ordre = False
                if local_min[-3] < local_max[-3] < local_min[-2] < local_max[-2] < local_min[-1] < local_max[-1]:
                    ordre = True
                # ----- condition pour que l'ordre des point de la figure soit respecter -----#

                # ----- condition pour que la tete fasse au minimum 2.8% -----#
                mini_pourcent = False
                if ((D - ((C + E) / 2)) * 100) / D >= 2.8:
                    mini_pourcent = True
                # ----- condition pour que la tete fasse au minimum 2.8% -----#

                # ----- condition pour garantir la forme de l'iete  -----#
                if (B - C) < (D - C) and (B - C) < (D - E) and (F - E) < (D - E) and (F - E) < (
                        D - C) and D > B and D > F and C < B and E < F and A <= mirande2['c'].iloc[
                    ...
                    if accept == True:
                        moyenne_epaule1 = (B - (I[1]) + (B - C)) / 2
                        moyenne_epaule2 = ((F - E) + (F - J[1])) / 2
                        moyenne_des_epaule = ((F - E) + (F - J[1])) + (B - (I[1]) + (B - C)) / 4
                        moyenne_tete = ((D - C) + (D - E)) / 2
                        # ----- creation variable des moyennes de la tete et epaules  pour les prochaines conditions-----#
                    ...
                    if I[1] < B and J[1] < F and moyenne_epaule1 <= moyenne_tete / 2 and moyenne_epaule2 <= moyenne_tete / 2 and moyenne_epaule1 >= moyenne_tete / 4 and moyenne_epaule2 >= moyenne_tete / 4 and accept == True and df['c'].values[-2] >= J[1] - (moyenne_tete) / 4 and df['c'].values[-2] <= J[1] and df['c'].values[-1] >= J[1] - (moyenne_tete) / 4 and df['c'].values[-1] <= J[1] and G >= 1:
                        ...
                        note = 0
                        pourcentage_10_tete = (10 * (local_min[-1] - local_min[-2])) / 100
                        pourcentage_10_ep1 = (20 * (local_min[-2] - I[0])) / 100
                        pourcentage_10_ep2 = (20 * (J[0] - local_min[-1])) / 100
                        pourcentage_20_moy_epaule = (30 * moyenne_des_epaule) / 100

                        debugage = []

                        if local_max[-2] < (local_min[-2] + local_min[-1]) / 2 + pourcentage_10_tete and local_max[
                            -2] > (local_min[-2] + local_min[
                            -1]) / 2 - pourcentage_10_tete:  # D doit etre au millieu (10% de marge)
                            note = note + 3
                            debugage.append(1)

                        if local_max[-3] < (I[0] + local_min[-2]) / 2 + pourcentage_10_ep1 and local_max[-3] > (
                                I[0] + local_min[-2]) / 2 - pourcentage_10_ep1:  # B doit etre au millieu (10% de marge)
                            note = note + 1
                            debugage.append(2)

                        if local_max[-1] < (J[0] + local_min[-1]) / 2 + pourcentage_10_ep2 and local_max[-1] > (
                                J[0] + local_min[-1]) / 2 - pourcentage_10_ep2:  # F doit etre au millieu (10% de marge)
                            note = note + 1
                            debugage.append(3)
                        ...
                        if (local_min[-1] - local_min[-2]) > local_min[-2] - I[0] and (local_min[-1] - local_min[-2]) > \
                                J[0] - local_min[-1]:  # tete plus large que les 2 epaules
                            note = note + 0.5
                            debugage.append(6)
                        ...
                        for i in range(int(round(J[0])), place_liveprice):
                            if df['l'].iloc[i] <= J[1] - (moyenne_tete) / 2 and dejatoucher == False:
                                dejatoucher = True
                                dejatoucher2 = 'OUI'
                        ...
                        # playsound('note.wav')
                        moins50p = G - ((moyenne_tete) * 60 / 100)
                        ...
                        pertenet = 0.005 * (1000/G)
                        if pertenet < 1:
                            pertenet = 1
                        if pertenet > 10:
                            pertenet = 10
                        pertenet = pertenet * 2  # 2 fois puisque maker et taker
                        pertenet_pourcent = (pertenet * 100) / 1000
